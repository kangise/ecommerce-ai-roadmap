...
## G3: Amazon 视频广告字幕规格（2026-08-13 已解决）

已按 Amazon Ads 官方 Sponsored Brands Video 规格与审核指南补入：默认静音、
无声可理解/字幕建议、站点语言、本地化字幕和右下角安全区。官方未公布统一
字幕字数上限，因此仓库明确不虚构该数值。约束见
`amazon.sponsored_brand.video.*`，正文见
`src/a-operators/a3-advertising.md#sponsored-brands-video-官方字幕与静音规则`。
