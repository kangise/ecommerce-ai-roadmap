...
1. [AI × 跨境电商：热度 vs 实际应用 Gap 矩阵](#ai--跨境电商热度-vs-实际应用-gap-矩阵)
2. [优先级分层：你应该从哪里开始？](#优先级分层你应该从哪里开始)
3. [AI Before vs After: 每个运营环节的真实变化](#ai-before-vs-after-每个运营环节的真实变化)
4. [你的 AI 落地路线图](#你的-ai-落地路线图)
5. [把这份评估套到你自己身上](#把这份评估套到你自己身上)
6. [常见误判](#常见误判)
7. [什么时候这套不管用](#什么时候这套不管用)
8. [接下来去哪里？](#接下来去哪里)
...
> **这一章的数字是怎么来的**
>
> 本章的成熟度评分、Before/After 工时和效率提升百分比，都是**基于实操的估算**，
> 不是抽样统计，也没有公开数据源可以核对——这类数据没有人公开发布。
>
> 它们的用途是**判断量级和优先级**：哪些环节值得先投入、大致能省下多少，
> 而不是拿去做预算测算或对外引用。你自己的数字会因品类、团队熟练度和
> 工具选型而明显不同，差一倍很正常。
>
> 涉及市场规模、平台份额这类**可以核实**的事实，本库的做法是逐条标注来源
> 并记录核验日期（见[平台全景对比](../d-platforms/platform-comparison.md)）。
> 本章不属于那一类，所以这里说清楚，不给它套一个查不到的引用。
...
POV: TikTok 的核心竞争力是"内容产量 x 内容质量"。AI 同时提升了两者: 产量大幅上去，质量因为 Hook 基于数据分析而不是凭感觉。但 AI 生成脚本 + 人工拍摄的组合效果最好。纯 AI 视频（数字人）适合标品但不适合需要信任感的品类。
...
这一页给的是"AI 在各环节现在能做到什么"，不是"你的团队现在该做什么"。两者的差距在于你的数据取不取得到、谁来做、错了谁兜底。把下面这段贴给 AI，用你自己的情况跑一遍：

```
<角色>跨境电商运营顾问，熟悉 AI 在各业务环节的成熟度差异</角色>
...
- 月销量级：[填]
- 团队人数与角色：[填]
- 已有数据接口：[SP-API / 只能手工导表 / 第三方工具，填具体的]
- 现在最花时间的三件事：[填]
</输入数据>

<任务>
1. 按"值不值得投入"给我最花时间的那三件事各打一个分，并说明理由
2. 每件事指出：需要什么数据、这个数据我现在取不取得到、取不到的话第一步该解决什么
3. 从中选出一件作为起点，说明为什么是它而不是另外两件
4. 对不建议现在做的，说清楚是"AI 还不成熟"还是"我的前置条件不具备"——这两种情况的应对完全不同

<数据纪律>
- 只使用我在<输入数据>里给出的信息。我没写的，问我，不要替我假设
- 不要引用具体的市场数据、行业均值或成熟度分数——你手上没有可核实的来源
- 每条建议标注：[基于我提供的信息] 或 [基于通用经验]
</数据纪律>
</任务>

<输出格式>
三件事各一段，每段包含：结论 / 缺什么 / 第一步。最后单独一行给出你的起点建议。
</输出格式>

<自检>
① 请求的 4 项（按"值不值得投入"给我最花时间的那三件事各打一个分，并说明理由 2. 每件事指出：需要什么数据、这个数据我现在取…）全部出现，编号与顺序和请求一致，无缺项无多余项。
