...
| L13 | RC | done | 1.3.0 版本合同；RC manifest fresh；pytest 245；14 groups=0；clean-clone JSON gate；Python 3.12 cold wheel + installed Demo/recovery smoke；browser console 0 | `loop(v1.3): L13 certify release candidate` | 确定性源码/UI/测试/分发 hash；tag gate；checkout 外冷安装；Release `SHA256SUMS`；L5 外部 block 已签收且 L6 零写入 |
...
- Date: 2026-08-26
- Status: done
- Evidence: `tests/test_release_candidate.py` 5 version/manifest/tamper/wheel/workflow/human-contract cases + clean-clone generated JSON tracking regression；全套 pytest 245 passed；`verify_all` 14 groups=0；`scripts/build_dist.py --check` 与 `scripts/build_release_manifest.py --check` fresh；Python 3.12 dependency wheelhouse 冷安装在 checkout 外验证 `opc-ecommerce 1.3.0`、MCP、Schema v21、Demo Daily Ops completed、human Proposal executed、Amazon Ads Proposal blocked、Eval/Security passed、backup verify-only 与新目标 restore；浏览器对安装 wheel 的 Demo 运行刷新持久化、真实 Security Assurance 再运行与 console 0；`artifacts/design-qa/l13-release-candidate-final.jpg`。
- Commit: `loop(v1.3): L13 certify release candidate`（本记录所在提交）
- Notes: 项目/package/OpenAPI/HTTP Server/User-Agent 统一为 `1.3.0`；确定性 RC manifest 绑定 package sources、UI assets、完整 scripts/tests 输入、`dist/`、wheel package data、Changelog、RC、roadmap 和本账本；现有 `verify_all` Dist 组阻断 stale manifest；tag 必须等于项目版本；Release workflow 在 Python 3.11 完成真实依赖冷安装和 checkout 外纵向 smoke，并为 wheel、zip、manifest、RC 文档生成 `SHA256SUMS`。L5 的外部 Amazon Ads block 已在 RC 签收，L6 继续无 adapter/route/action/write operation。
