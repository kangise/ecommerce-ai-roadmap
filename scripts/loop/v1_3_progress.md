...
| L13 | RC | done | 1.3.0 版本合同；RC manifest fresh；pytest 246；14 groups=0；clean-clone JSON gate；L7 UI recertified at 1440/1024/390/320；Python 3.12 cold wheel + installed Demo/recovery smoke；browser console 0 | `loop(v1.3): L13 certify release candidate` | 确定性源码/UI/测试/分发 hash；NovoChoice L7 UI adaptation；tag gate；checkout 外冷安装；Release `SHA256SUMS`；L5 外部 block 已签收且 L6 零写入 |
