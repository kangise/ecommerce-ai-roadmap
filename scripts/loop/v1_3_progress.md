...
| L12 | eval/security/restore | done | Schema v21；239 tests；14 groups=0；wheel backup/restore/corruption smoke；browser Assurance persistence | `loop(v1.3): L12 verify assurance and recovery` | audit hash chain；fenced Assurance；WAL-consistent backup；no-overwrite restore；external encryption required |
| L13 | RC | done | 1.3.0 版本合同；RC manifest fresh；pytest 247；14 groups=0；clean-clone JSON gate；L7 UI recertified at 1440/1024/390/320 + explicit Light/Dark/hover contrast；Python 3.12 cold wheel + installed Demo/recovery smoke；browser console 0 | `loop(v1.3): L13 certify release candidate` | 确定性源码/UI/测试/分发 hash；NovoChoice L7 UI adaptation；persistent theme control；tag gate；checkout 外冷安装；Release `SHA256SUMS`；L5 外部 block 已签收且 L6 零写入 |
...
### L12 — eval / security / restore

- Date: 2026-08-26
- Status: done
- Evidence: `tests/test_assurance.py` 5 audit/eval/security/idempotency/fencing cases + `tests/test_recovery.py` 6 online backup/restore/corruption/rollback/path/CLI cases；全套 pytest 239 passed；`verify_all` 14 groups=0；OpenAPI closed Audit/Assurance contracts；Python 3.12 wheel cold install + Demo seed + online backup + verify-only + new-target restore + restored Assurance/audit-chain + corrupted-backup rejection/no-partial-target smoke；browser seeded Eval/Security, real rerun, reload persistence, visible hash links, console 0 errors；`artifacts/design-qa/l12-assurance-recovery-final.jpg`。
- Commit: `loop(v1.3): L12 verify assurance and recovery`（本记录所在提交）
- Notes: Schema v21 backfills each tenant's existing audit history into a deterministic SHA-256 previous/event hash chain；SQLite triggers verify new links, actor ownership and closed shape, then reject update/delete；Assurance eval/security runs are admin-triggered, viewer-readable, idempotent, lease-token/attempt fenced, terminal/audit atomic, and explicitly blocked when no eligible workflow data exists；Eval accepts only the latest `weekly-ops-v3` result for eligible Reviewer-approved lineage；Security checks DB/FK, chain, required triggers and persisted credential values. Backup uses SQLite online backup, includes committed WAL state, copies only snapshot-referenced immutable Evidence objects, self-verifies manifest hashes/schema/FKs/audit chains, and writes 0700/0600 output. Restore is verify-first, no-overwrite, objects-first/database-last, rollback-safe, and records per-tenant restore Assurance + system audit.
- Validator results: all repository/contract/install/browser gates passed；installed restore preserved 10 Evidence objects, Schema 21 and a valid audit chain；a tampered DB checksum returned stable `RESTORE_FAILED` with no target left behind.
- Failure 1: adversarial review found cross-tenant audit actors, fresh-vs-migrated shape drift, stale Assurance attempts able to race terminal writes, ambiguous mixed failed/blocked severity, and evaluation coverage accepting any historical pass. Actor/shape/hash triggers, 300-second lease tokens with attempt fencing, failed-first aggregation, and latest/current evaluator checks now reject those paths.
- Failure 2 / blocked reason: recovery review found snapshot/object races, recursive output overlap, DB-first partial install, unverified staging/WAL state, giant CLI manifest output, and an over-strict ancestor-symlink rule that rejected standard macOS `/tmp`. The final implementation copies snapshot references only, canonicalizes parents while rejecting final/data symlinks, checkpoints/restages and fault-rolls back, outputs bounded summaries, and passes a real symlink-parent alias test. The backup directory contains sensitive business data and is not encrypted by this package；production must add external encryption, key management and access control. No L12 implementation blocker remains.
- Restore point and result: L11 `3aff785`; L12 Schema v21/Audit chain/Assurance/API/UI/Recovery CLI can be reverted as one commit while preserving L0–L11 business tables. Backups remain independently verifiable directories；restore never overwrites an existing runtime target.

### L13 — Release Candidate

- Date: 2026-08-26
- Status: done
- Evidence: `tests/test_release_candidate.py` 5 version/manifest/tamper/wheel/workflow/human-contract cases + clean-clone generated JSON tracking regression + `tests/test_ui.py` L7 decision-workspace/font/token/theme/a11y/AA contrast contract；全套 pytest 247 passed；`verify_all` 14 groups=0；`scripts/build_dist.py --check` 与 `scripts/build_release_manifest.py --check` fresh；Python 3.12 dependency wheelhouse 冷安装在 checkout 外验证 `opc-ecommerce 1.3.0`、MCP、Schema v21、Demo Daily Ops completed、human Proposal executed、Amazon Ads Proposal blocked、Eval/Security passed、backup verify-only 与新目标 restore；NovoChoice L7 UI adaptation 在 1440/1024/390/320px 验证 Canvas/详情转换、0 overflow、字体 loaded、7 个工作区和关键交互、console 0；显式 Light/Dark 控制、刷新持久、theme-color/chart 同步，以及 Primary/Amazon/TikTok/Agent/identity 的 Dark hover/active 对比均有浏览器 computed-style、AA contrast guard 和同视口截图；`design-qa.md` final result passed，最终对照位于 `artifacts/design-qa/l7-ui-comparison-*.png` 与 `l7-theme-comparison-*.png`。
- Commit: `loop(v1.3): L13 certify release candidate`（本记录所在提交）
- Notes: 项目/package/OpenAPI/HTTP Server/User-Agent 统一为 `1.3.0`；确定性 RC manifest 绑定 package sources、UI assets、完整 scripts/tests 输入、`dist/`、wheel package data、Changelog、RC、roadmap 和本账本；现有 `verify_all` Dist 组阻断 stale manifest；tag 必须等于项目版本；Release workflow 在 Python 3.11 完成真实依赖冷安装和 checkout 外纵向 smoke，并为 wheel、zip、manifest、RC 文档生成 `SHA256SUMS`。RC 后的 UI recertification 采用 NovoChoice L7 原则与视觉：自托管 IBM Plex/Source Han、400/500 weights、4/6/8px geometry、neutral selection、surface-first boundaries、explicit persistent Light/Dark、role-specific Dark icon contrast、accessibility variants、Canvas-first responsive behavior；API/RBAC/actions/SSE/persistence 未改变。L5 的外部 Amazon Ads block 已在 RC 签收，L6 继续无 adapter/route/action/write operation。
- Validator results: all repository、contract、supported-Python wheel、installed runtime、recovery and browser gates passed；refresh/reopen 后状态仍持久，未观察到控制台错误。
- Failure 1: 初版 RC manifest 仅覆盖 Python 文件与部分测试输入，且 workflow 未给最终 wheel 生成摘要；终验前已扩展至 UI、全部 scripts/tests、合同文档，并加入所有 Release assets 的 `SHA256SUMS` 后重建验证。终审进一步发现 8 个必需生成 JSON 与 RC manifest 本身被全局 `*.json` 忽略、manifest 未绑定 wheel 的 README/license metadata、CI 只 verify-only 而没有实际 restore、非 draft Release 可能在附件上传失败时部分公开；这些 clean-clone/发布缺口现已由 tracking 回归、动态构建输入 hash、installed absent-target restore + Assurance/audit/object assertions，以及 draft→上传→下载 checksum→公开流程关闭。工作区 Python 3.12 初始未预装 pytest，首次完整依赖下载又超时；仅装最小依赖后的第一次全量测试据实出现 2 个 pandas 缺失的 Notebook 失败。UI recertification 的首次全量测试也按设计被 stale RC manifest 门禁阻断；更新全部 UI/证据合同并重建 manifest 后，从头重跑为 246 passed。
- Failure 2 / blocked reason: 当前环境仍无真实 Amazon Ads 凭据/批准，也未提供 seller-authorized SP-API 或生产 OpenAI 凭据；因此未执行或宣称这些外部 live 成功。该限制按 RC 合同保留，L5 状态仍为 external `blocked`，L13 本身无实现阻塞。制品签名/SBOM、外部备份加密与多进程/HA 也不在本轮声明范围。
- Restore point and result: L12 `1f864e0`；L13 仅冻结版本、发布合同、门禁与证据，不迁移 Schema v21。回退应用可回到 L12；数据恢复继续使用经校验的 absent-target restore，绝不覆盖现有数据库。
