...
- **手順が固定のとき。** 毎回同じ順序、同じ分岐 — それに必要なのは Agent ではなくワークフローである。Agent のコストは、次に何をするかをモデルに毎回決め直させるところから来るもので、その判断が実際に変わるときにだけ価値がある。固定手順は n8n のようなツールのほうが安く、予測もしやすい([F5](../0-foundations/f5-rpa-automation.md) 参照)。
