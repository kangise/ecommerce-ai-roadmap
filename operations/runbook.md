...
## L6 Ads adapter status

`GET /v1/ads-adapter-status` is a tenant-scoped read-only status endpoint.
This release intentionally has no Ads write adapter, operation, route, or
action. Treat `eligible_not_installed` as “gate evidence is fresh, but no
adapter exists”, never as permission to write. The 24-hour L5 freshness,
account-update time, required capabilities, Profile, and region checks must all hold. Future writes require
proposal, approval, audit, idempotency, and rollback controls.
