      ...
      "mcpServers": {
        "opc-ecommerce": {
          ...
          "args": ["/path/to/ecommerce-ai-skills/integration/mcp-server.py",
                   "--dist", "/path/to/ecommerce-ai-skills/dist"]
        }
      }
    }

Resources (read-only data the agent can query):
    opc://ontology/entities        — 100 e-commerce domain entities
    opc://ontology/constraints      — 322 platform constraints
...
# The normal server mode requires the MCP SDK.  ``--cli`` and ``--validate``
# remain dependency-light diagnostic modes; there is no silent protocol fallback.
try:
...
class OPCServer:
    ...
    def _validate_package_files(self) -> dict:
        ...
        if not manifest_path.is_file():
            raise PackageValidationError(
                f"missing {manifest_path}; run: python3 scripts/build_dist.py"
            )
        try:
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise PackageValidationError(f"invalid {manifest_path}: {exc}") from exc
        if manifest.get("schema_version") != 1:
            raise PackageValidationError("unsupported package manifest schema")
        hashes = manifest.get("sha256")
        if not isinstance(hashes, dict) or not hashes:
            raise PackageValidationError("package manifest has no file hashes")
        for rel, expected in hashes.items():
            path = (self.dist / rel).resolve()
            try:
                path.relative_to(self.dist)
            except ValueError as exc:
                raise PackageValidationError(f"unsafe manifest path: {rel}") from exc
            if not path.is_file():
                raise PackageValidationError(f"package file missing: {rel}")
            actual = hashlib.sha256(path.read_bytes()).hexdigest()
            if actual != expected:
                raise PackageValidationError(f"package file checksum mismatch: {rel}")
        return manifest
    ...
    def _load_json(self, name: str) -> Any:
        ...
        try:
            ...
            raise PackageValidationError(f"cannot load {name}: {exc}") from exc
    ...
    def _load_text(self, name: str) -> str:
        ...
        try:
            ...
            raise PackageValidationError(f"cannot load {name}: {exc}") from exc
        if not text.strip():
            raise PackageValidationError(f"package file is empty: {name}")
        return text
    ...
    def _load_skills(self) -> dict[str, dict]:
        ...
        if not skills_dir.is_dir():
            raise PackageValidationError("package directory missing: skills/")
        for sd in sorted(skills_dir.iterdir()):
            ...
            try:
                ...
                raise PackageValidationError(f"invalid skill manifest {mf}: {exc}") from exc
            if not isinstance(manifest, dict) or not manifest.get("name"):
                raise PackageValidationError(f"invalid skill manifest structure: {mf}")
            sid = manifest["name"]
            skills[sid] = {
                "manifest": manifest,
                "playbook": self._required_text(sd / "references" / "playbook.md"),
                "constraints": self._required_text(sd / "references" / "constraints.md"),
                "boundaries": self._required_text(sd / "references" / "boundaries.md"),
            }
        return skills

    @staticmethod
    def _required_text(p: Path) -> str:
        try:
            text = p.read_text(encoding="utf-8")
        except OSError as exc:
            raise PackageValidationError(f"required package file missing: {p}") from exc
        if not text.strip():
            raise PackageValidationError(f"required package file is empty: {p}")
        return text

    def _validate_loaded_counts(self) -> None:
        counts = self._package_manifest.get("counts") or {}
        actual = {
            "chapters": len(self._knowledge) if isinstance(self._knowledge, list) else -1,
            "entities": len(self._ontology.get("entities", [])) if isinstance(self._ontology, dict) else -1,
            "relations": len(self._ontology.get("relations", [])) if isinstance(self._ontology, dict) else -1,
            "constraints": len(self._ontology.get("constraints", [])) if isinstance(self._ontology, dict) else -1,
            "processes": len(self._ontology.get("processes", [])) if isinstance(self._ontology, dict) else -1,
            "platforms": len(self._ontology.get("platforms", [])) if isinstance(self._ontology, dict) else -1,
            "prompts": len(self._prompts) if isinstance(self._prompts, list) else -1,
            "skills": len(self._skills),
        }
        for key, value in actual.items():
            expected = counts.get(key)
            if not isinstance(expected, int) or expected <= 0 or value != expected:
                raise PackageValidationError(
                    f"package count mismatch for {key}: manifest={expected}, loaded={value}"
                )
        ...
        for sid, data in self._skills.items():
            ...
            for pattern in patterns:
                try:
                    ...
                    raise PackageValidationError(
                        f"{sid}: invalid routing pattern {pattern!r}: {exc}"
                    ) from exc
            rules.append({
                "skill": sid,
                "keywords": keywords,
    ...
    def _score_skills(self, query: str) -> list[tuple[str, int]]:
        ...
        for rule in self._routing_rules:
            ...
            score += 2 * sum(
                1 for pattern in rule["patterns"] if re.search(pattern, qn, re.IGNORECASE)
            )
            if score:
...
def main() -> int:
    ...
    parser.add_argument("--validate", action="store_true", help="Validate package and exit")
    ...
    try:
        ...
        print(f"Error: invalid dist package: {exc}", file=sys.stderr)
        return 1
    ...
    if not HAS_MCP:
        print(
            'Error: MCP SDK is not installed; run: pip install "mcp>=1.28,<2"',
            file=sys.stderr,
        )
        return 2
